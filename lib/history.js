	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
		...
		else
		{
			...
			if (this._items.length === 0 ||
				(this._items.length > 0 && block.timestamp > this.worstBlockTimestamp()) ||
				(this._items.length < MAX_HISTORY && block.timestamp < this.worstBlockTimestamp() && addingHistory) )
			{
...
History.prototype._save = function(block)
{
	...
	this._items = _.sortByOrder( this._items, 'block.timestamp', false );
...
{
	return _.max(this._items, 'block.timestamp');
}
...
{
	return _.min(this._items, 'block.timestamp');
}
...
History.prototype.getNodePropagation = function(id)
{
	...
	var sorted = _( this._items )
		.sortByOrder('block.timestamp', false)
	...
	if(this._callback !== null)
	{
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
				...
				var difficulty = (item.block.difficulty << 8) + sealer
				return {
					height: item.height,
		...
		this._callback(null, {
			...
			sealer : _.pluck( chartHistory, 'sealer' ),
...
History.prototype.getHistoryRequestRange = function()
{
	...
	var blocks = _.pluck( this._items, 'block.timestamp' );
