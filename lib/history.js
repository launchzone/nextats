...
var ORDER_FIELD = 'block.timestamp';
...
History.prototype.add = function(block, id, trusted, addingHistory)
{
	...
	if( !_.isUndefined(block) && !_.isUndefined(block.number) && !_.isUndefined(block.uncles) && !_.isUndefined(block.transactions) && !_.isUndefined(block.difficulty) && block.number > 0 )
	{
		...
		historyBlock = false; // ignore chain forks
		if( historyBlock )
		{
		...
		else
		{
			...
			if( this._items.length === 0 || (this._items.length > 0 && block.timestamp > this.worstBlockTimestamp()) || (this._items.length < MAX_HISTORY && block.timestamp < this.worstBlockTimestamp() && addingHistory) )
			{
...
History.prototype._saveAndReplace = function(block) {
	var index = _.findIndex( this._items, { height: block.height } );

	if (index >= 0) {
		// found exist number
		if (this._items[index].timestamp < block.timestamp) {
			// replace
			this._items[index] = block;
		}
		return
	}
	this._save(block);
...
History.prototype._save = function(item)
{
	this._items.unshift(item);

	this._items = _.sortByOrder( this._items, ORDER_FIELD, false );
	...
	if (!_.isUndefined(item.block.price) && item.block.price.length > 0) {
		this._prices.unshift(item);
		this._prices = _.sortByOrder(this._prices, ORDER_FIELD, false);
		if (this._prices.length > MAX_HISTORY)
		{
			this._prices.pop();
		}
	}
...
{
	return _.max(this._items, ORDER_FIELD);
}
...
{
	return _.min(this._items, ORDER_FIELD);
}
...
History.prototype.worstBlockTimestamp = function(trusted)
{
	var worst = this.worstBlock();

	if( !_.isUndefined(worst.block.timestamp) )
		return worst.block.timestamp;
...
History.prototype.getStablePrice = function()
{
	var stablePrice = _( this._items )
		.sortBy(ORDER_FIELD, false )
		.filter(function (item)
		{
			return !_.isUndefined(item.block.price) && item.block.price.length > 0;
		})
		.slice(0, MAX_BINS)
		.reverse()
		.map(function (item)
		{
			if (item.block.price === '0') {
				return undefined;
			}
			return parseFloat(item.block.price) - 1.0;
		})
		.value();

	// always keep the zero value in view
	stablePrice[0] = undefined;

	return stablePrice;
}

History.prototype.getMedianPrice = function()
{
	var medianPrice = _(this._prices)
		.sortByOrder(ORDER_FIELD, false)
		.filter(function (item)
		{
			return !_.isUndefined(item.block.priceMedian) && item.block.priceMedian.length > 0;
		})
		.slice(0, MAX_BINS)
		.reverse()
		.map(function (item)
		{
			if (item.block.priceMedian === '0') {
				return undefined;
			}
			return parseFloat(item.block.priceMedian) - 1.0;
		})
		.value();
	
	// always keep the zero value in view
	medianPrice[0] = undefined;

	return medianPrice;
}

History.prototype.getToAbsorb = function()
{
	var toAbsorb = _( this._items )
		.sortByOrder(ORDER_FIELD, false)
		.filter(function (item)
		{
			return !_.isUndefined(item.block.toAbsorb) && item.block.toAbsorb.length > 0;
		})
		.slice(0, MAX_BINS)
		.reverse()
		.map(function (item)
		{
			if (item.block.toAbsorb === '0') {
				return undefined;
			}
			return item.block.toAbsorb;
		})
		.value();
	
	// always keep the zero value in view
	toAbsorb[0] = undefined;

	return toAbsorb;
}

History.prototype.getAbsorbed = function()
{
	var absorbed = _( this._items )
		.sortByOrder(ORDER_FIELD, false)
		.filter(function (item)
		{
			return !_.isUndefined(item.block.absorbed) && item.block.absorbed.length > 0;
		})
		.slice(0, MAX_BINS)
		.reverse()
		.map(function (item)
		{
			if (item.block.absorbed === '0') {
				return undefined;
			}
			return item.block.absorbed;
		})
		.value();
	
	// always keep the zero value in view
	absorbed[0] = undefined;

	return absorbed;
}

History.prototype.getSupplySTB = function()
{
	var supplySTB = _( this._items )
		.sortByOrder(ORDER_FIELD, false)
		.filter(function (item)
		{
			return !_.isUndefined(item.block.supplySTB) && item.block.supplySTB.length > 0;
		})
		.slice(0, MAX_BINS)
		.reverse()
		.map(function (item)
		{
			if (item.block.supplySTB === '0') {
				return undefined;
			}
			return item.block.supplySTB;
		})
		.value();
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		var chartHistory = _( this._items )
			...
			.map(function (item)
			{
				var sealer = parseInt(item.block.miner.substring(22, 22+2), 16)
				var difficulty = Number(item.block.difficulty) + sealer / 256.0
				difficulty = Number(difficulty.toFixed(2)).toFixed(3);
				if (item.block.nonce === 0) {
					// mark the prioritized block for directives.js's sparkchart colorMap
					difficulty = (Number(difficulty) + 0.005).toFixed(3);
				}
		...
		// 	chartHistory[0].difficulty = 0;
		// }

		this._callback(null, {
			height : _.pluck( chartHistory, ORDER_FIELD ),
			...
			stablePrice: this.getStablePrice(),
			medianPrice: this.getMedianPrice(),
			toAbsorb: this.getToAbsorb(),
			absorbed: this.getAbsorbed(),
			supplySTB: this.getSupplySTB(),
...
History.prototype.getHistoryRequestRange = function()
{
	...
	var blocks = _.pluck( this._items, ORDER_FIELD );
